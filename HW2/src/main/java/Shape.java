...
public class Shape {
    ...
    public boolean is_cross (Shape other)
    {
        double distance1;
        double distance2;
        Circle circleA = this.circle;
        Circle circleB = other.circle;
        Vector<Point> pointA = points;
        Point heart_other =circleB.getCenter();
        double radiusB = circleB.getRadius();
        for(int i=0;i<points.size()-1;i++){
            distance1 = heart_other.distance(points.get(i));
            distance2 = heart_other.distance(points.get(i+1));
    ...
    {
        Circle other_Circle = other.getCircle();
        double distance = circle.getCenter().distance(other_Circle.getCenter());
        if(distance + other_Circle.getRadius() < circle.getRadius())
        {
            return 2;
        }
        else if (distance <= circle.getRadius() + other_Circle.getRadius())
        {
            return 1;
        }
        else {
