...
public class Piece {
    ...
    public Piece(TPoint[] points) {
        ...
        Calculate_HeightAndWidth();
        Calculate_Skirt();
    }
    private void Calculate_HeightAndWidth()
    {
        int Max_x = 0;
        int Max_y = 0;
        for (TPoint index : body)
        {
            if(Max_x < index.x) Max_x = index.x;
            if(Max_y < index.y) Max_y = index.y;
        }
        width = Max_x + 1;
        height = Max_y + 1;
    }

    private void Calculate_Skirt()
    {
        skirt = new int[width];
        Arrays.fill(skirt,height - 1);
        for (TPoint index : body)
        {
            if(skirt[index.x] > index.y) skirt[index.x] = index.y;
        }
    }
    /**
    ...
    public Piece computeNextRotation() {
        int n = body.length;
        TPoint[] rotate = new TPoint[n];
        for (int i = 0 ; i < n ; i++)
        {
            rotate[i] = new TPoint(height - body[i].y - 1, body[i].x);
        }
        Piece Cal_Next = new Piece(rotate);
        return Cal_Next;
    }
    ...
    public boolean equals(Object obj) {
        ...
        List<TPoint> PointsOfMine = Arrays.asList(body);
        List<TPoint> OtherPoint = Arrays.asList(other.body);
        return PointsOfMine.containsAll(OtherPoint);
    }
	...
	*/
    private static Piece makeFastRotations(Piece root) {
        Piece piece_portion = new Piece(root.body);
        Piece rootOfMine = piece_portion;
        while (true)
        {
            Piece next_rotate = piece_portion.computeNextRotation();
            if(rootOfMine.equals(next_rotate))
            {
                piece_portion.next = rootOfMine;
                break;
            }
            piece_portion.next = next_rotate;
            piece_portion = piece_portion.next;
        }
        return rootOfMine;
    }
